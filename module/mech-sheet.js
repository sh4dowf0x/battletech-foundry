...
function countFerroFibrousCritSlots(actorSystem) {
  ...
  for (const loc of Object.values(crit)) {
    ...
    for (const s of iter) {
      ...
      if (/ferro\s*-?\s*fibrous/i.test(label)) slots += 1;
    }
  }
...
async function buildAutoWeaponsFromCritSlots(actor) {
  ...
  for (const locKey of locKeys) {
    ...
    for (let i = 0; i < slots.length; i++) {
      ...
      candidates.push({ locKey, index: i, uuid, label, span, destroyed });
    }
  }
...
// ------------------------------------------------------------
const HEAT_CRITICAL_SFX = `systems/${SYSTEM_ID}/assets/heat-critical.mp3`;
const HEAT_MODERATE_SFX = `systems/${SYSTEM_ID}/assets/heat-moderate.mp3`;
const SHUTDOWN_SFX = `systems/${SYSTEM_ID}/assets/shutting-down.mp3`;
const SYSTEMS_NOMINAL_SFX = `systems/${SYSTEM_ID}/assets/systems-nominal.mp3`;
const STARTING_UP_SFX = `systems/${SYSTEM_ID}/assets/starting-up.mp3`;
...
const WEAPON_DESTROYED_SFX = `systems/${SYSTEM_ID}/assets/weapon-destroyed.mp3`;
const FOOTSTEPS_SFX = `systems/${SYSTEM_ID}/assets/footsteps.mp3`;
const JUMPJET_FAILURE_SFX = `systems/${SYSTEM_ID}/assets/jumpjet-failure.mp3`;
const COOLANT_FAILURE_SFX = `systems/${SYSTEM_ID}/assets/coolant-failure.mp3`;
const DAMAGE_CRITICAL_SFX = `systems/${SYSTEM_ID}/assets/damage-critical.mp3`;
...
function maybePlayArmorBreachedSfx(actor) {
  ...
  playAtowSfx(ARMOR_BREACHED_SFX, { volume: 0.95 });
}
...
function maybePlayWeaponDestroyedSfx(actor) {
  ...
  playAtowSfx(WEAPON_DESTROYED_SFX, { volume: 1.0 });
}
...
function maybePlayJumpjetFailureSfx(actor) {
  ...
  playAtowSfx(JUMPJET_FAILURE_SFX, { volume: 1.0 });
}
...
function maybePlayCoolantFailureSfx(actor) {
  ...
  playAtowSfx(COOLANT_FAILURE_SFX, { volume: 1.0 });
}
...
function maybePlayDamageCriticalSfx(actor) {
  ...
  playAtowSfx(DAMAGE_CRITICAL_SFX, { volume: 1.0 });
}
...
function maybePlayEjectionExplosionSfx(actor) {
  ...
  setTimeout(() => {
    ...
    if (!noEject) playAtowSfx(EJECTION_EXPLOSION_SFX, { volume: 1.0 });
  }, 1500);
}
  ...
  for (const locKey of ["lt", "rt"]) {
    ...
    for (let i = 0; i < maxPossible; i++) {
      ...
      if (i < desiredSide) {
        ...
        if (!uuid) {
          if (label !== "Engine") updates[`system.crit.${locKey}.slots.${i}.label`] = "Engine";
        ...
        if (!uuid && label === "Engine") updates[`system.crit.${locKey}.slots.${i}.label`] = "";
      }
    }
  }
...
function _defaultCritLabel(actor, locKey, idx) {
  ...
  if ((lk === "lt" || lk === "rt") && Number.isFinite(i)) {
    ...
    if (side > 0 && i >= 0 && i < side) return "Engine";
  }
...
function isActorShutdown(actor) {
  ...
  try {
    if (actor?.statuses?.has) hasShutdownStatus = actor.statuses.has("atow.shutdown");
    else if (Array.isArray(actor?.statuses)) hasShutdownStatus = actor.statuses.includes("atow.shutdown");
  } catch (_) {}
...
async function clearActorShutdown(actor, tokenDoc) {
  ...
  try {
    ...
    try {
      if (!desiredShutdown && actor?.statuses?.has) desiredShutdown = actor.statuses.has("atow.shutdown");
      if (!desiredShutdown && Array.isArray(actor?.statuses)) desiredShutdown = actor.statuses.includes("atow.shutdown");
    } catch (_) {}
...
if (!globalThis.__ATOW_BT_SFX_REGISTERED__) {
  ...
  // Primary: start-of-turn / start-of-round transitions
  Hooks.on("updateCombat", (combat, changed) => {
    ...
    // Defer slightly so heat venting automation (which often also runs on updateCombat)
    // has time to update actor.system.heat before we read it.
    setTimeout(async () => {
      maybePlayHeatSfxForActor(actor, combat);
...
export class AToWMechSheet extends ActorSheet {
  /** @override */
  static get defaultOptions() {
    const opts = super.defaultOptions ?? {};
    return {
      ...opts,
      classes: [...(opts.classes ?? []), "atow", "battletech", "sheet", "mech"],
      template: "systems/atow-battletech/templates/mech-sheet.hbs",
      width: 1200,
      height: 900,
      submitOnChange: true,
      submitOnClose: true,
  ...
  async getData(options) {
    ...
    context.tsm = {
      ...
      mascPresent,
      needed: tsmNeeded,
      slotsTotal: tsmSlotsTotal,
      slotsIntact: tsmSlotsIntact,
      heat: tsmHeat,
      heatThreshold: tsmHeatThreshold
    };
    ...
    context.derivedMoveBase = derivedMoveBase;
    ...
    system.movement = system.movement ?? {};
    system.movement.jumpJets = context.jumpJets;
    ...
    const meleeEntries = buildMeleeAttackEntries(this.actor, { tsmActive: Boolean(context?.tsm?.active) });
    ...
    // Physical attacks (simple editable structure for now)
    const physicalAttacks = system.physicalAttacks ?? {
      punch: { type: "Punch", loc: "Arm", dmg: 0 },
      kick: { type: "Kick", loc: "Leg", dmg: 0 },
      charge: { type: "Charge", loc: "N/A", dmg: 0 }
    };
...
const hasFerro = ferroSlots >= ferroNeeded;
...
const ferroArmorMultiplier = (techBase === "clan") ? 1.20 : 1.12;
const armorTons = roundTons(hasFerro
  ? (armorPoints / (16 * ferroArmorMultiplier))
  : armorTonsStd);


const armorNote = hasFerro
  ? `Ferro-Fibrous (${ferroSlots}/${ferroNeeded} slots)`
  : `Standard (${ferroSlots}/${ferroNeeded} slots)`;
...
const gyroTons = engineRating ? Math.ceil(engineRating / 100) : 0;
...
const _requestedEngineSinks = Number(system?.heat?.engineSinks);
...
context.tonnageBreakdown = [
  ...
  { key: "gyro",       label: "Gyroscope",  tons: gyroTons,      display: `${gyroTons}t`,      note: engineRating ? `ceil(${engineRating}/100)` : "—" },
  ...
  { key: "other",      label: "Other",       tons: otherTons,     display: `${otherTons}t`,     note: "Equipment w/ tonnage fields (incl. crit-slot gear)" }
    ...
    context.physicalAttacks = Object.entries(physicalAttacks).map(([key, value]) => ({
      key,
      ...value
    }));
...
const buildSlots = (locKey, labels, offset = 0) => {
  ...
  for (let i = 0; i < total; i++) {
    ...
    // Destroyed flag is per-slot so multi-slot components can take crits on any occupied slot.
    const destroyed = Boolean(stored?.destroyed);
...
if ($engSlider.length && $engNumber.length) {
  ...
  $engNumber.on("input change", (ev) => {
    ...
    html.find(".drop-zone").on("drop", (ev) => ev.preventDefault()); // actual creation handled by _onDrop
  }
  ...
  async _onItemEdit(event) {
    ...
    if (uuid) {
      try {
        ...
        if (item?.sheet) return item.sheet.render(true);
      } catch (_) {
        /* empty */
      }
    }
    ...
    // Synthetic melee attacks (not Items)
    if (itemId && String(itemId).startsWith("melee-")) {
      const meleeType = (String(itemId) === "melee-punch") ? "punch" : "kick";
      return promptAndRollMeleeAttack(this.actor, meleeType);
    }
